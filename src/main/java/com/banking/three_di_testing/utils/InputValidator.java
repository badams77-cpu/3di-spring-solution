...
public class InputValidator {
    ...
    public static boolean isTransactionSortAndAccountCriteriaValid(TransactionsByDateInput accountInput) {
        return constants.SORT_CODE_PATTERN.matcher(accountInput.getSortCode()).find() &&
                constants.ACCOUNT_NUMBER_PATTERN.matcher(accountInput.getAccountNumber()).find();
    }
    ...
    public static boolean isTransactionsByDateValid(TransactionsByDateInput transactionsByDateInput) {
        // TODO Add checks for large amounts; consider past history of account holder and location of transfers

        if (!isTransactionSortAndAccountCriteriaValid(transactionsByDateInput))
            return false;
        if (transactionsByDateInput.getStartDate()==null){
            return false;
        }
        if (transactionsByDateInput.getEndDate()==null){
            return false;
        }
